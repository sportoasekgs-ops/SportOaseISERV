...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_booking') {
        ...
        if ($date && $period && !empty($students)) {
            ...
            header('Location: /test/?success=booking_created');
            exit;
        }
    }
    ...
    if ($action === 'delete_booking') {
        ...
        header('Location: /test/?success=booking_deleted');
        exit;
    }
    
    // Block slot (admin only)
    if ($action === 'block_slot' && $isAdminUser) {
        $date = $_POST['date'] ?? '';
        $period = (int)($_POST['period'] ?? 0);
        ...
        if ($date && $period) {
            ...
            header('Location: /test/?success=slot_blocked');
            exit;
        }
    }
}
        ...
        <div class="container mx-auto px-4 py-6">
            <div class="flex justify-between items-center">
                <div class="flex items-center gap-4">
                    <img src="/public/assets/icon-192.png" alt="SportOase" class="w-12 h-12 rounded-lg shadow">
                    <div>
    ...
    <div id="bookingModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <form method="POST" class="p-8">
                ...
                <h2 class="text-2xl font-bold text-gray-800 mb-6">Neue Buchung</h2>
                
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Angebot / Aktivität</label>
                    <input type="text" name="offer" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                </div>
                
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-4">Schüler (max. 5)</label>
                    <div class="space-y-3">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                            <div class="flex gap-3">
                                <input type="text" name="student<?= $i ?>_name" placeholder="Name" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg">
                                <input type="text" name="student<?= $i ?>_class" placeholder="Klasse" class="w-32 px-4 py-2 border border-gray-300 rounded-lg">
                            </div>
                        <?php endfor; ?>
                    </div>
                </div>
                ...
                <div class="flex gap-4">
                    <button type="submit" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-6 rounded-lg">
                        Buchung erstellen
                    </button>
                    <button type="button" onclick="closeBookingModal()" class="px-6 py-3 text-gray-600 hover:text-gray-800">
                        Abbrechen
                    </button>
                </div>
            </form>
